...
class TreeState:
    ...
    def interactive_elements_to_string(self) -> str:
        return "\n".join(
            [
                f"Label: {index} App Name: {node.app_name} ControlType: {f'{node.control_type} Control'} Name: {node.name} Shortcut: {node.shortcut} Cordinates: {node.center.to_string()}"
    ...
    def informative_elements_to_string(self) -> str:
        return "\n".join(
            [
                f"App Name: {node.app_name} Name: {node.name}"
    ...
    def scrollable_elements_to_string(self) -> str:
        ...
        return "\n".join(
            [
                f"Label: {n+index} App Name: {node.app_name} ControlType: {f'{node.control_type} Control'} Name: {node.name} Cordinates: {node.center.to_string()} Horizontal Scrollable: {node.horizontal_scrollable} Vertical Scrollable: {node.vertical_scrollable}"
